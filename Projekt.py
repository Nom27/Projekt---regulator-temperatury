...
def update_plot(frame):
    ...
    if line:
        try:
            ...
            ax1.legend()
            ax1.grid()
            ax1.set_xlim(left=0, right=max(time_data) + 1)  # Oś X od 0 do maksymalnego czasu
            ...
            ax2.legend()
            ax2.grid()
            ax2.set_xlim(left=0, right=max(time_data) + 1)  # Oś X od 0 do maksymalnego czasu

        except Exception as e:
            print(f"Error parsing line: {line} -> {e}")

# Funkcja do wysyłania komend
def send_commands():
    while True:
        command = input("Wprowadź komendę (np. T010): ").strip()
        if command:
            try:
                ser.write((command + '\n').encode('utf-8'))
...
# Tworzenie wątku dla wysyłania komend
